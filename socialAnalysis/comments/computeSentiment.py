        ...
        if str(row[2]) in userids:
            if not country_restricted or str(genderdata[str(row[2])]["country"]) in country_restrictions:
                try:
                    ...
                    data[str(globalindex)] = {"user": str(row[2]), "afterChange": int(timestamp >= changedate), "sentiment": float(cur_sentiment), "timestamp": str(timestamp)}
                    globalindex += 1
                except:
        ...
        if str(row[1]) in userids:
            if not country_restricted or str(genderdata[str(row[1])]["country"]) in country_restrictions:
                try:
                    ...
                    data[str(globalindex)] = {"user": str(row[1]), "afterChange": int(timestamp >= changedate), "sentiment": float(cur_sentiment), "timestamp": str(timestamp)}
                    globalindex += 1
                except:
                    logging.warning("Could not read following line:")
                    logging.warning(str(row))
