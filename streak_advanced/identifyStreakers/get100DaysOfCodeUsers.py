...
# ---------- OUTPUT ------------
path_results = "/home/lmoldon/results/100DaysOfCodeUsers.json"
# ------------------------------
...
while(not done):
    try:
        ...
        forks = answer.json()
        if len(forks) > 0:
            for cur_user_data in forks:
                cnt_users += 1
                if cnt_users % 100 == 0:
                    logging.info(str(cnt_users) + " users computed")
                usersearch = getUserIdFromLogin(cur_user_data["owner"]["login"])
                if usersearch != -1:
                    cnt_identified += 1
                    data[str(usersearch)] = cur_user_data["owner"]["login"]
            page += 1
            time.sleep(0.72)
        else:
            done = True
